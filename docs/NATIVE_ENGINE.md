...
The helper identifies the exact `pppN` or `utunN` interface created by each supervised process. Profile CIDRs are added only after that process reports a completed tunnel, and disconnect or failure removes only the routes owned by that session. Separate sessions therefore retain separate interfaces when multiple VPNs are connected concurrently.

OpenVPN authentication is handled through a profile-specific Unix management socket inside the root-only runtime directory. Ordinary username/password, static `SCRV1`, dynamic `CRV1` and password-plus-OTP flows are supported. The UI exposes OTP only after the engine reaches its challenge state. Management values are escaped before transmission, the socket is removed with its session and authentication material is cleared after use.
