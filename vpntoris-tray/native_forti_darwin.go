...
const nativeHelperSocket = "/var/run/vpntoris-native/helper.sock"
...
func nativeHelperReady() bool {
	info, err := os.Stat(nativeHelperSocket)
...
func nativeFortiLogs(name string) ([]byte, error) {
	path := filepath.Join("/var/log/vpntoris", nativeProfileID(name)+".log")
...
func nativeFortiRequest(request fortihelper.Request) (*fortihelper.Response, error) {
	...
	connection, err := net.DialTimeout("unix", nativeHelperSocket, 2*time.Second)
	if err != nil {
		return nil, err
	}
